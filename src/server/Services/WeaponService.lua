...
function WeaponService:Start()
    ...
    table.insert(self._connections, fireWeaponRemote.OnServerEvent:Connect(function(player, targetPosition)
...
function WeaponService:PerformRaycast(player: Player, targetPosition: Vector3, weaponData: WeaponData): {[string]: any}
    local character = player.Character
    if not character then
        return { hit = false }
    end
    ...
    local direction = (targetPosition - origin).Unit * weaponData.range
